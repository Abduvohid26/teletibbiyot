...
function genderDisplayLabel(label: string, t: (key: string) => string): string {
  const normalized = label.trim().toLowerCase();
  if (normalized === 'male' || normalized === 'erkak' || label === 'MALE') return t('gender.male');
  if (normalized === 'female' || normalized === 'ayol' || label === 'FEMALE') return t('gender.female');
  return label;
}
...
export function UtAnalyticsContent({ className }: UtAnalyticsContentProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (failures.length === results.length) {
        ...
        throw new Error(msg instanceof Error ? msg.message : t('analytics.loadFailed'));
      }
      if (failures.length > 0) {
        const msg = failures[0]?.reason;
        setError(msg instanceof Error ? msg.message : t('analytics.partialLoadFailed'));
      }
  ...
  const periodOptions = PERIOD_OPTIONS.map((o) => ({ value: o.value, label: t(o.labelKey) }));
    ...
    <div className={cn('space-y-5', className)}>
      ...
      {overview?.scopeLabel && (
        <p className="text-xs text-slate-500 bg-white/60 border border-white/80 rounded-lg px-3 py-2">
          {t('analytics.viewLabel')} <span className="font-medium text-slate-700">{overview.scopeLabel}</span>
        </p>
      )}
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-slide-up">
            <MetricCard icon={Activity} label={t('analytics.totalConsultations')} value={overview.totalConsultations} color="brand" />
            <MetricCard icon={CheckCircle2} label={t('analytics.completed')} value={overview.completed} sub={`${overview.completionRate}%`} color="emerald" />
            <MetricCard icon={Clock} label={t('analytics.queuedInProgress')} value={overview.queued + overview.inProgress} color="amber" />
            <MetricCard icon={Users} label={t('analytics.patientsPeriod')} value={overview.totalPatients} color="cyan" />
            <MetricCard icon={Brain} label={t('analytics.aiAnalyses')} value={overview.withAiAnalysis} color="violet" />
            <MetricCard icon={Target} label={t('analytics.finalDiagnosis')} value={overview.withFinalDiagnosis} color="indigo" />
            <MetricCard
              icon={Timer}
              label={t('analytics.avgDuration')}
              value={overview.avgDurationMinutes ?? '—'}
              suffix={overview.avgDurationMinutes ? t('analytics.minutes') : ''}
              color="slate"
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-slide-up">
            <div className="panel p-5">
              <h3 className="panel-title mb-4 flex items-center gap-2">
                <TrendingUp size={16} className="text-brand-600" /> {t('analytics.trendTitle')}
              </h3>
              <TrendChart data={trends} />
            </div>
            <div className="panel p-5">
              <h3 className="panel-title mb-4 flex items-center gap-2">
                <AlertTriangle size={16} className="text-amber-600" /> {t('analytics.triageDistribution')}
              </h3>
          ...
          <div className="panel p-5 animate-slide-up">
            <h3 className="panel-title mb-4 flex items-center gap-2">
              <Brain size={16} className="text-violet-600" /> {t('analytics.topAiDiagnoses')}
            </h3>
            <BarChart
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 animate-slide-up">
            {aiInsights && (
              <div className="panel p-5">
                <h3 className="panel-title mb-4">{t('analytics.aiInsights')}</h3>
                <div className="space-y-3 text-sm">
                  <InsightRow label={t('analytics.totalAnalyses')} value={String(aiInsights.totalAnalyses)} />
                  <InsightRow label={t('analytics.avgConfidence')} value={`${aiInsights.avgConfidence}%`} />
                  <InsightRow
                    label={t('analytics.diagnosisMatch')}
                    value={aiInsights.diagnosisMatchRate !== null ? `${aiInsights.diagnosisMatchRate}%` : '—'}
                  />
                  <InsightRow label={t('analytics.redFlags')} value={String(aiInsights.redFlagCases)} highlight />
                </div>
              </div>
            )}
            {demographics && (
              <>
                <div className="panel p-5">
                  <h3 className="panel-title mb-4">{t('analytics.byGender')}</h3>
                  <BarChart
                    data={demographics.gender.map((g) => {
                      const label = genderDisplayLabel(g.label, t);
                      const isMale = label === t('gender.male') || g.label === 'MALE' || g.label === 'Erkak';
                      return {
                        label,
                        value: g.value,
                        color: isMale ? 'bg-blue-500' : 'bg-pink-500',
                      };
                    })}
                  />
                </div>
                <div className="panel p-5">
                  <h3 className="panel-title mb-4">{t('analytics.byRegion')}</h3>
                  <BarChart
