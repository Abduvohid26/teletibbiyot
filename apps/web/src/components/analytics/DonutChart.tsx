...
import { TRIAGE_COLORS, TRIAGE_LABEL_KEYS } from '@/lib/analytics-types';
...
export function DonutChart({ data }: DonutChartProps) {
  ...
  if (total === 0) {
    return <p className="text-sm text-slate-400 text-center py-8">{t('analytics.noData')}</p>;
  }
      ...
      >
        <div className="absolute inset-3 bg-white rounded-full flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-[9px] text-slate-500 uppercase">{t('common.all')}</p>
          </div>
        </div>
      </div>
        ...
        {data.map((d) => (
          <div key={d.level} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-slate-600 flex-1">
              {TRIAGE_LABEL_KEYS[d.level] ? t(TRIAGE_LABEL_KEYS[d.level]) : d.level}
            </span>
