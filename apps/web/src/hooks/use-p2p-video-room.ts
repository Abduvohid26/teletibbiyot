...
export function useP2PVideoRoom({
  consultationId,
  role,
  enabled = true,
  onCallEnded,
  ...
  const lastAdaptModeRef = useRef<'normal' | 'degraded' | 'min_layer'>('normal');
  ...
  const makeOffer = useCallback(
    async (remoteSocketId: string, force = false) => {
      ...
      try {
        ...
        pc = createPeerConnection(remoteSocketId, true);
  ...
  useEffect(() => {
    ...
    const onVisible = () => {
      ...
      const anyBad = [...pcsRef.current.values()].some(
        (pc) => pc.connectionState === 'disconnected' || pc.connectionState === 'failed',
      );
      if (anyBad) runRecovery();
    };
  ...
  useEffect(() => {
    ...
    const onReconnect = () => {
      // Signalizatsiya socket id o'zgaradi, media yo'li o'zgarmaydi.
      // Google Meet ham signaling reconnect'da PeerConnection'ni yopmaydi —
      // yopish qora ekran. Faqat o'lik PC'larni tozalab, ICE restart + room sync.
      ...
      deadIds.forEach((id) => {
        ...
        socketToCamerasRef.current.delete(id);
  ...
  useEffect(() => {
    ...
    if (q === 'poor') {
      ...
      // ~6s past sifat — pasaytirish; ~16s — min qatlam (lekin video O'CHMAYDI)
      if (poorStreakRef.current >= 8) {
        if (lastAdaptModeRef.current !== 'min_layer') {
          lastAdaptModeRef.current = 'min_layer';
          void applyAudioPriorityMode(pcsRef.current, 'min_layer');
        }
        setNetworkAudioOnly(true);
      } else if (poorStreakRef.current >= 3) {
        if (lastAdaptModeRef.current !== 'degraded') {
          lastAdaptModeRef.current = 'degraded';
    ...
    if (q === 'fair') {
      if (lastAdaptModeRef.current === 'min_layer' && goodStreakRef.current < 3) return;
      qualityPresetRef.current = 'low';
      if (lastAdaptModeRef.current !== 'degraded') {
        lastAdaptModeRef.current = 'degraded';
        void applyAllSendersBitrate(pcsRef.current, 'low');
      ...
      if (networkAudioOnlyRef.current) setNetworkAudioOnly(false);
      return;
    }

    // good/excellent — sekin tiklash (Meet: drop fast, recover slow)
    if (goodStreakRef.current < 4) return;
    if (networkAudioOnlyRef.current) setNetworkAudioOnly(false);
    if (lastAdaptModeRef.current !== 'normal') {
      lastAdaptModeRef.current = 'normal';
